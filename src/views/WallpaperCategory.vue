    ...
    <div class="card flex justify-center">
      ...
      <div v-if="images" class="grid grid-cols-12 gap-4 w-full" >
        <div v-for="(item, index) of data" :key="index" class="col-span-3">
          <img :src="getServerSource(item.source)" :alt="item.name" class="cursor-pointer w-full h-full object-contain shadow-xl rounded-md"  @click="()=>router.push({name:'WallpaperDetail',params:{id:item._id}})" />
        </div>
      </div>
    </div>
  </div>
