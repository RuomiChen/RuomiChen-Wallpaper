...
<template>
  <CardContainer @click="router.push({name:'CenterOther',params:{id:data.user_data._id}})" :turn="false" class="cursor-pointer relative overflow-hidden h-[362px] w-full grid grid-cols-2 gap-4 p-4" v-if="data">
    ...
    <CardBody
      class="z-2 absolute left-[50%] top-[50%] -translate-x-[50%] -translate-y-[50%]  group/card size-24    bg-contain  ">
      <CardItem class="border-white border-2 rounded-full w-full h-full overflow-clip" scale="1.4fea" >
